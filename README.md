# Salary Calculator (Google Calendar Integration)
...
**[アプリを開く（本番環境）](https://my-salary-app-fur7.onrender.com)**

### 初回ログイン時のご注意
本アプリは個人のカレンダーを読み取るため、Googleの公式審査を受ける前の状態となっています。

そのため、ログイン時に**「Google はこのアプリを確認していません」**という警告画面が表示されますが、以下の手順でテスト利用が可能です。

1. 左下の **「詳細」** をクリックする
2. 一番下の **「〇〇（アプリ名）に移動（安全ではないページ）」** をクリックする
...
本アプリは、Googleカレンダーの**予定のタイトル（名前）**から時間を読み取ります。
